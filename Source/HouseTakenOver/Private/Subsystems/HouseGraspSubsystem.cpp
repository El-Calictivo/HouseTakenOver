...
void UHouseGraspSubsystem::RegisterPossessorOrb(APossessorOrb* NewPossessor)
{
	if (!NewPossessor)
	{
		return;
	...
	PossessionOrbs.AddUnique(NewPossessor);
}
...
{
	for (auto PivotRoom : PivotRooms)
	{
		GraspAdjacentRooms(PivotRoom, GraspAmount);
	}
}
...
ARoom* UHouseGraspSubsystem::GetRandomRoom(const ERoomState RoomState)
{
	...
	if (!RoomCollection || RoomCollection->IsEmpty())
	{
		return nullptr;
	}
	...
	for (uint8 i = 0; i < ERoomStatePtr->NumEnums() - 1; ++i)
	{
		...
		if (RoomCollection)
		{
			for (auto Room : *RoomCollection)
			{
				if (Room)
				{
					Room->OnRoomStateChanged.RemoveDynamic(this, &UHouseGraspSubsystem::OnRoomStateChanged);
				}
			}
