...
void ATakenHouseGameMode::OnRoomStateChanged(ARoom* Room, const ERoomState OldState, ERoomState NewState)
{
	...
	if (NewState == ERoomState::TAKEN)
	{
		...
		OnRoomTaken(Room);
		if (HouseGraspSubsystem->GetRoomCollection(ERoomState::BEING_TAKEN)->IsEmpty())
		{
			OnLastRoomTaken(Room);
		}
	}
}
...
{
	IncreaseAdjacentRoomsGraspBy(ERoomState::SEALED,
...
void ATakenHouseGameMode::IncreaseAdjacentRoomsGraspBy(const ERoomState State, const float GraspAmount)
{
	...
	HouseGraspSubsystem->GraspAdjacentRooms(*HouseGraspSubsystem->GetRoomCollection(ERoomState::TAKEN),
	                                        GraspAmount * HouseGraspSubsystem->GetCurrentGraspForceMultiplier());
}
